...
//		frames.forEach(frame -> System.out.println(frame.url()));
		...
		firstFrame.getByPlaceholder("Enter name").type("Kevin");
		FrameLocator nestedFrame = firstFrame.frameLocator("iframe.has-background-white");
		nestedFrame.getByPlaceholder("Enter email").type("Kevin@mail.com");
		
		firstFrame.getByPlaceholder("Enter name").fill("Kevin Ortiz");
